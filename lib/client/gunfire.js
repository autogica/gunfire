...
window.ObjectPoolFactory = (function() {
  function ObjectPoolFactory(size, buildOptions) {
    this.size = size != null ? size : 1000;
    this.buildOptions = buildOptions != null ? buildOptions : {};
  ...
  ObjectPoolFactory.prototype.addToScene = function(scene) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      obj = _ref[_i];
      _results.push(this.scene.add(obj.mesh));
    }
    return _results;
  };

  ObjectPoolFactory.prototype.compile = function(newConfig) {
    var buildOptionsChanged, geometryFactoryChanged, i, len, materialFactoryChanged, mesh, meshFactoryChanged, obj, objectFactoryChanged, sizeChanged, _i, _j, _len, _ref, _ref1, _ref2, _ref3;
    ...
    geometryFactoryChanged = false;
    materialFactoryChanged = false;
    meshFactoryChanged = false;
    objectFactoryChanged = false;
    ...
    if ((newConfig.materialFactory != null) && ("" + newConfig.materialFactory) !== ("" + this.materialFactory)) {
      ...
      materialFactoryChanged = true;
      meshFactoryChanged = true;
      objectFactoryChanged = true;
    }
    ...
    if ((newConfig.buildOptions != null) && ("" + (JSON.stringify(newConfig.buildOptions))) !== ("" + (JSON.stringify(this.buildOptions)))) {
      ...
      buildOptionsChanged = true;
      geometryFactoryChanged = true;
      materialFactoryChanged = true;
      meshFactoryChanged = true;
      objectFactoryChanged = true;
    }
    if (geometryFactoryChanged && (((_ref = this.geometry) != null ? _ref.dispose : void 0) != null)) {
      console.log("ObjectPoolFactory: compile: disposing of old geometry..");
      this.geometry.dispose();
    }
    if (materialFactoryChanged && (((_ref1 = this.material) != null ? _ref1.dispose : void 0) != null)) {
    ...
    this.geometry = this.geometryFactory(this.buildOptions);
    console.log("ObjectPoolFactory: compile: building new material..");
    this.material = this.materialFactory(this.geometry, this.buildOptions);
    if (this.compiled && this.meshChanged) {
    ...
    if (this.objects.length >= this.size) {
      ...
      for (i = _j = len, _ref3 = this.size; len <= _ref3 ? _j < _ref3 : _j > _ref3; i = len <= _ref3 ? ++_j : --_j) {
        mesh = this.meshFactory(this.geometry, this.material, this.buildOptions);
        mesh.visible = false;
        if (this.addedToScene) {
          this.scene.add(mesh);
        }
  ...
  ObjectPoolFactory.prototype.get = function(instanceFactory, opts) {
    var obj;
